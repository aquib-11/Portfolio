...
useEffect(() => {
  if (isVisible && ulRef.current) {
    const scrollDuration = 500; // duration in milliseconds
    const intervalDuration = 5; // interval in milliseconds
    const totalScrollDistance =
      ulRef.current.scrollWidth - ulRef.current.clientWidth;
    const step = totalScrollDistance / (scrollDuration / intervalDuration);

    let currentScroll = ulRef.current.scrollLeft;
    const interval = setInterval(() => {
      if (currentScroll < totalScrollDistance) {
        currentScroll += step;
        ulRef.current.scrollLeft = currentScroll;
      } else {
  ...
  return (
    <Wrapper ref={navRef} user={user} isVisible={isVisible}>
      <ul ref={ulRef}>
        {isVisible && (
          <>
            ...
            {links.map((link) => {
              ...
              return (
                <li key={text}>
                  <NavLink to={path} end onClick={handleNavLinkClick}>
            ...
            {user.userRole === "admin" && (
              <li key={privateLink.text}>
                <NavLink to={privateLink.path} onClick={handleNavLinkClick} end>
        ...
        <span className="navHam" onClick={()=>{  setIsVisible(!isVisible)}}>
